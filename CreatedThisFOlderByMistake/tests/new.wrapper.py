...
frame_idx = [
    pickle.load(open(i, "rb")) for i in glob("*frame_numbers.pkl") if pickle.load(open(i, "rb"))
]

for name in glob("*frame_numbers.pkl"):
    vec, frame_idx = (
        pickle.load(open(name.replace("frame_numbers", "vecs"), "rb")),
        pickle.load(open(name, "rb"))
    )

'''
Converting the annotations to frame_rows and corresponding frame_idxs
'''
